...
var cssPath = path.join(path.normalize(`${__dirname}`), 'main.css');
var usagePath = path.join(path.normalize(`${__dirname}/tpl`), 'Header.tpl');

var createCssObject = function (cssPath, callback) {
    fs.readFile(cssPath, {encoding: 'utf-8'}, (err, data) => {
        if (!err){
            var css_object = {};
            var parsed = css.parse(data).stylesheet.rules;

            var all_elements = _.flatten(_.pluck(parsed, 'selectors'));
            var all_classes = all_elements.filter((el) => /^\./.test(el));
            all_classes.map((el) => {
                var el_classes = /[A-Za-z_.]+/.exec(el)[0].split('.');
